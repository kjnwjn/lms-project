<template>
  <div class="h-screen md:flex">
    ...
    >
      <div>
        <h1 class="text-white font-bold text-4xl font-sans">Go to Home Page</h1>
        <p class="text-white mt-1">Best Your Choice</p>
        <router-link to="/">
          <button
    ...
    <div class="flex md:w-1/2 justify-center py-10 items-center bg-white">
      <form @submit.prevent="handleSubmit" class="bg-white">
        ...
        <p v-if="passwordMatch" style="color: red">Passwords do not match!</p>
        <button
        ...
        <p class="text-center text-gray-600">
          ...
          <router-link to="/login">
            <span class="text-indigo-600">Login</span></router-link
          >
        </p>
      </form>
    </div>
  ...
  methods: {
    async handleSubmit() {
      try {
        ...
        const response = await userService.studentRegister(data);
        console.log(response);
        this.$toast.success("Register successfully!");
        this.$router.push("/login");
      } catch (error) {
        console.log(error);
        this.$toast.error("Register failed! Please try again!");
      }
    },
  },
};
